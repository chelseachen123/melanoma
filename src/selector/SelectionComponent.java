...
public class SelectionComponent extends JComponent implements MouseListener, MouseMotionListener,
    ...
    private void paintLiveWire(Graphics g) {
        PolyLine lastSegment = model.liveWire(mouseLocation);

        g.setColor(liveWireColor);
        g.drawPolyline(lastSegment.xs(),lastSegment.ys(), lastSegment.size());
    ...
    private void paintControlPoints(Graphics g, List<PolyLine> segments) {
        int diagonal = (int) (controlPointRadius / Math.sqrt(2));

        for (PolyLine segment : segments) {
            g.setColor(controlPointColor);
            g.fillOval(segment.xs()[0] - diagonal, segment.ys()[0] - diagonal,
    ...
    private void paintMoveGuides(Graphics g, List<PolyLine> segments) {
        ...
        Point controlPoint = segments.get(selectedIndex).start();
        Point before = (selectedIndex > 0) ? segments.get(selectedIndex - 1).start()
                : segments.getLast().start();
        Point after = (selectedIndex < segments.size() - 1) ? segments.get(selectedIndex + 1).start()
                : segments.getFirst().start();

        int[] x = {before.x, mouseLocation.x, after.x};
        int[] y = {before.y, mouseLocation.y, after.y};
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        if (e.getButton() == 1 &&  (model.state() == SELECTING || model.state() == NO_SELECTION)) {
            model.addPoint(e.getPoint());
        } else if (e.getButton() == 2 && model.state() == SELECTING) {
            model.finishSelection();
        } else if (e.getButton() == 3 &&
                (model.state() == SELECTING || model.state() == SELECTED)) {
            model.undo();
        }
    }
    ...
    public void mousePressed(MouseEvent e) {
        if (e.getButton() == MouseEvent.BUTTON1 && model.state() == SELECTED) {
            updateMouseLocation(e.getPoint());
            selectedIndex = model.closestPoint(mouseLocation, controlPointRadius);
        }
