...
public class ScissorsWeights {
    ...
    static Weigher<ImageEdge> makeWeigher(String weightName, ImageGraph graph) {
        return switch (weightName) {
            ...
            // DONE
            case "CrossGradMulti" -> new CrossGradMultiWeight(graph);
    ...
    static Iterable<String> weightNames() {
        return List.of("CrossGradMono", "CrossGradMulti");
    }
     ...
     * image.  Only considers the brightness of a grayscale (band-averaged) version of the image.
     */
    static class CrossGradMultiWeight implements Weigher<ImageEdge> {
        ...
        /**
         * The image represented by `graph`.
         */
         ...
         * Create a new weigher capable of weighing edges in `graph`.
         */
        CrossGradMultiWeight(ImageGraph graph) {
            this.graph = graph;
        ...
        public int weight(ImageEdge edge) {
            ...
            for (int b = 0; b < image.getNumBands(); b++) {
                eGradMax -= crossGrad(image, x, y, b, edge.dir()) / image.getNumBands();
            }
            return eGradMax;
        }
    }
